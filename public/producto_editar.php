...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($categoria) || $precio <= 0) {
        ...
        $stmt = $pdo->prepare("UPDATE productos SET nombre = ?, descripcion = ?, cantidad = ?, precio = ?, categoria = ? WHERE id = ?");
        if ($stmt->execute([$nombre, $descripcion, $cantidad, $precio, $categoria, $id])) {
                ...
                <div class="form-card-body">
                    ...
                    <form method="POST">
                        <div class="mb-3 input-group-icon">
                            ...
                            <input type="text" name="nombre" class="form-control" placeholder="Nombre del producto *" required value="<?= htmlspecialchars($producto['nombre']) ?>">
                        </div>

                        <div class="mb-3 input-group-icon">
                            <i class="fas fa-align-left"></i>
                            <textarea name="descripcion" class="form-control" rows="3" placeholder="Descripción (opcional)"><?= htmlspecialchars($producto['descripcion']) ?></textarea>
                        </div>

                        <div class="row">
                            <div class="col-md-6 mb-3 input-group-icon">
                                <i class="fas fa-boxes"></i>
                                <input type="number" name="cantidad" class="form-control" placeholder="Cantidad" value="<?= $producto['cantidad'] ?>" min="0">
                            </div>
                            <div class="col-md-6 mb-3 input-group-icon">
                        ...
                        <div class="mb-4 input-group-icon">
                            ...
                            <input type="text" name="categoria" class="form-control" placeholder="Categoría *" required value="<?= htmlspecialchars($producto['categoria']) ?>">
                        </div>
